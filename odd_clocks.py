...
class SimulatorClock(tk.Canvas):
    ...
    def render_puzzle(self):
        ...
        if mode == "Triangular":
            ...
            clock_r, marker_r, hull_r, lobe_hull_r, spacing = 32, 38, 330, 58, 88
            self.clock_pos = [( (c-2.5)*spacing, (r-2.5)*spacing ) for r in range(6) for c in range(6)]
            lobe_pos = [(-215, -215), (215, -215), (-215, 215), (215, 215)]
            ...
            clock_r, marker_r, hull_r, lobe_hull_r, spacing = 28, 33, 350, 40, 78
            self.clock_pos = [( (c-3)*spacing, (r-3)*spacing ) for r in range(7) for c in range(7)]
            lobe_pos = [(-240, -240), (240, -240), (-240, 240), (240, 240)]
            ...
            clock_r, marker_r, hull_r, lobe_hull_r = 32, 38, 260, 60
            lobe_dist, self.pin_radius = 215, 14
            PIN_DIST, OUTER_CLOCK_R, INNER_CLOCK_R, offset_angle = 130, 215, 160, 22.5

            lobe_pos = [(lobe_dist * math.cos(math.radians(i * 45 - 90 + offset_angle)),
                         lobe_dist * math.sin(math.radians(i * 45 - 90 + offset_angle))) for i in range(8)]

            p_order = list(range(8)) if self.is_front else [0, 7, 6, 5, 4, 3, 2, 1]
            self.pin_coords = [(PIN_DIST * math.cos(math.radians(i * 45 - 90 + offset_angle)),
                                PIN_DIST * math.sin(math.radians(i * 45 - 90 + offset_angle))) for i in p_order]

            self.pin_labels = ["O1", "O2", "O3", "O4", "O5", "O6", "O7", "O8"]
            self.clock_pos = []
            for i in range(8):
                a = math.radians(i * 45 - 90 + offset_angle)
                self.clock_pos.append((OUTER_CLOCK_R * math.cos(a), OUTER_CLOCK_R * math.sin(a)))
            for i in range(8):
        ...
        for i, (dx, dy) in enumerate(self.clock_pos):
            ...
            if mode == "Triangular":
                ...
                if i in self.corner_map.values():
                    ...
                    self.draw_external_buttons(cx, cy, clock_r, ckey, btn_r=(11 if mode=="3x3" else 8), font_size=(11 if mode=="3x3" else 8), offset=(40 if mode=="6x6" else (36 if mode=="7x7" else (48 if mode=="3x3" else 38))))
            elif mode == "Octagonal" and i < 8:
                self.draw_external_buttons(cx, cy, clock_r, self.pin_labels[i], offset=50)
            elif i < 5 and mode not in ["Triangular", "Octagonal"] and "x" not in mode:
        ...
        for i, (px, py) in enumerate(self.pin_coords):
            ...
            elif mode == "Octagonal": mirror_map = {0:7,1:0,2:1,3:2,4:3,5:4,6:5,7:6}
            else: mirror_map = None
            
            actual_idx = i if self.is_front else (mirror_map[i] if mirror_map else ([0, 2, 1][i] if mode == "Triangular" else [0, 1, 2, 3, 4][i]))
            is_up = self.app.pin_states[actual_idx] if self.is_front else not self.app.pin_states[actual_idx]
            p_circ = self.create_oval(pcx-self.pin_radius, pcy-self.pin_radius, pcx+self.pin_radius, pcy+self.pin_radius, fill=(self.pin_up_color if is_up else self.pin_down_color), outline="#000000", width=2, tags="pin")
            
            # Render Label for Octagonal (and others)
            if mode == "Octagonal" or mode == "Triangular" or mode == "Pentagonal":
                self.create_text(pcx, pcy - self.pin_radius - 10, text=self.pin_labels[i], fill="#ff007f", font=("Arial", 10, "bold"), tags="pin_label")
                
            self.tag_bind(p_circ, "<Button-1>", lambda e, idx=actual_idx: self.toggle_pin(idx))

        self.tag_raise("pin"); self.tag_raise("btn"); self.tag_raise("pin_label")
...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.modes = ["Triangular", "3x3", "Pentagonal", "Super-Pentagonal", "Octagonal", "4x4", "5x5", "6x6", "7x7"]
    ...
    def change_mode(self, mode):
        ...
        p_count = 3 if mode == "Triangular" else (4 if mode == "3x3" else (8 if mode == "Octagonal" else (9 if mode == "4x4" else (16 if mode == "5x5" else (25 if mode=="6x6" else (36 if mode=="7x7" else 5))))))
        c_count = 6 if mode == "Triangular" else (9 if mode == "3x3" else (16 if mode == "Octagonal" else (16 if mode == "4x4" else (25 if mode == "5x5" else (36 if mode=="6x6" else (49 if mode=="7x7" else (11 if mode=="Super-Pentagonal" else 10)))))))
    ...
    def rotate_official(self, move_key, delta, is_front_click):
        ...
        elif mode == "Octagonal": self.rotate_octagonal(move_key, delta, is_front_click)
    ...
    def rotate_octagonal(self, move_key, delta, is_front_click):
        p = self.pin_states
        f_idx, b_idx = set(), set()
    ...
    def scramble(self):
        # Reset timer state
        self.timer_running = False
        self.is_waiting_for_first_move = True
        self.is_scrambling = True
        self.timer_label.configure(text="00:00.00")

        mode = self.mode_var.get()
        
        # 1. Scaling the move count based on puzzle surface area
        # More clocks = more moves needed to reach high entropy
        if mode == "Triangular":
            f_keys = ["U", "L", "R"]
            scramble_len = 20
        elif "Octagonal" in mode:
            f_keys = [f"O{i+1}" for i in range(8)]
            scramble_len = 50 
        elif "Pentagonal" in mode:
            f_keys = ["FU", "FUR", "FDR", "FDL", "FUL"]
            scramble_len = 35
        else: # Grid clocks 3x3 to 7x7
            f_keys = ["C1", "C2", "C3", "C4"]
            grid_size = int(mode.split('x')[0]) if 'x' in mode else 3
            # 3x3: 30 moves, 7x7: 80 moves
            scramble_len = 20 + (grid_size * 10 - 10) + 20

        # WCA Turn range: -5 to +6 (excluding 0)
        turn_values = [v for v in range(-5, 7) if v != 0]

        # 2. Sequential Movement Simulation
        # We simulate a human/machine turning the dials. 
        # This guarantees the state remains within the puzzle's mathematical group.
        for _ in range(scramble_len):
            # A) Randomize ALL pins for this specific turn
            # This ensures complex "bandaged" movements 
            self.pin_states = [random.choice([True, False]) for _ in range(len(self.pin_states))]
            
            # B) Select a side and a knob randomly
            side = random.choice([True, False])
            move_key = random.choice(f_keys)
            
            # C) Select a random rotation amount
            delta = random.choice(turn_values)
            
            # D) Execute the move through the official rotation logic
            self.rotate_official(move_key, delta, side)

        # 3. Final Random Pin State 
        # Sets the physical pins to a random position for the user to start
        self.pin_states = [random.choice([True, False]) for _ in range(len(self.pin_states))]
